...
DrawingBoard.Board.prototype = {
	...
	putImg:function(src){
		var ctx=this.ctx,width=ctx.canvas.width,height=ctx.canvas.height;
		var imgData=ctx.createImageData(width,height);
		for(var i=0,len=src.length;i<len;i++){
			imgData.data[i]=src[i];
			}
		ctx.putImageData(imgData,0,0);
		},
	drawCircle:function(src){
		if(src&&src.r&&src.g&&src.y){
			var redResult=src.r,greenResult=src.g,yellowResult=src.y,ctx=this.ctx,temp;
			if(redResult&&redResult.length>0){
				ctx.strokeStyle="#ffffff";
				for(var i=0,len=redResult.length;i<len;i++){
					temp=redResult[i];
					/*ctx.beginPath();
					ctx.save();
					ctx.lineWidth=1;
					ctx.arc(temp.x,temp.y,40,0,Math.PI*2,true);
					ctx.stroke();
					ctx.restore();
					*/
					ctx.globalCompositeOperation="source-over";
					...
					ctx.save();
					ctx.lineWidth=2;
					ctx.moveTo(temp.x-15*Math.sqrt(2),temp.y-15*Math.sqrt(2));
					ctx.lineTo(temp.x+15*Math.sqrt(2),temp.y+15*Math.sqrt(2));
					ctx.moveTo(temp.x-15*Math.sqrt(2),temp.y+15*Math.sqrt(2));
					ctx.lineTo(temp.x+15*Math.sqrt(2),temp.y-15*Math.sqrt(2));
					ctx.stroke();
					ctx.restore();
			...
			if(greenResult&&greenResult.length>0){
				ctx.strokeStyle="#ffffff";
				for(var i=0,len=greenResult.length;i<len;i++){
					temp=greenResult[i];
					ctx.globalCompositeOperation="source-over";
					ctx.beginPath();
					ctx.save();
					ctx.lineWidth=2;
					ctx.moveTo(temp.x,temp.y-27);
					ctx.lineTo(temp.x,temp.y+27);
					ctx.stroke();
					ctx.restore();
					}
				}
			if(yellowResult&&yellowResult.length>0){
				ctx.strokeStyle="#000000";
				for(var i=0,len=yellowResult.length;i<len;i++){
					temp=yellowResult[i];
				ctx.globalCompositeOperation="source-over";
				    ctx.beginPath();
					ctx.save();
					ctx.lineWidth=3;
					ctx.moveTo(temp.x-30,temp.y);
					ctx.lineTo(temp.x+30,temp.y);
					ctx.stroke();
					ctx.restore();
					}
				}
	...
	drawFace:function(faces){
		if(faces instanceof Array&&faces.length>0){
		var ctx=this.ctx,face;
	...
	setBackImg:function(src){
		...
		img.onload = function() {
			...
			if(img.width>ctx.canvas.width||img.height>ctx.canvas.height){
				window.currentWidth=ctx.canvas.width,window.currentHeight=ctx.canvas.height,window.currentOffsetX=0,window.currentOffsetY=0;
				ctx.drawImage(img,0,0,img.width,img.height,0, 0,ctx.canvas.width,ctx.canvas.height);
				}
			else
			{
			window.currentWidth=img.width,window.currentHeight=img.height,window.currentOffsetX=0,window.currentOffsetY=0;
			ctx.drawImage(img,0,0);
			}
			ctx.globalCompositeOperation = oldGCO;
		};
	...
	_onCanvasDrop: function(e) {
		...
		fr.onload = $.proxy(function(ev) {
			this.setBackImg(ev.target.result);
